...
function parseImport(string, pageProps) {
	...
	if (!attrs.use) {
		warning(pageProps.sys.href, '<import> elements require a `use` attribute whose value is a path to the file being imported in its place.');
		return inner;
	}

	let location = pageProps.sys.href;
	let importElementChildren = inner.match(matchTag());
	let importContent = readLocal(getAbsolutePath(attrs.use, location));
	let importProps = {};
